...
public class DocumentStoreImpl implements DocumentStore {
    ...
    private final MinHeap<Document> minHeap;
    ...
    private void textDocumentPut(URI uri, byte[] bytes) {
        ...
        minHeap.insert(documentText);
        minHeap.reHeapify(documentText);
    ...
    private void binaryDocumentPut(URI uri, byte[] bytes) {
        ...
        minHeap.insert(documentBinary);
        minHeap.reHeapify(documentBinary);
    ...
    private void removeFromEverything() {
        ...
        while (overTheLimit()) {
            ...
            if (leastUsedDoc.getDocumentTxt() != null) {
                docBytes -= leastUsedDoc.getDocumentTxt().getBytes().length;
                Set<String> words = leastUsedDoc.getWords();
                for (String word : words) {
                    documentTrie.delete(word, leastUsedDoc);
                }
            } else {
                docBytes -= leastUsedDoc.getDocumentBinaryData().length;
            }
            docCount -= 1;
            documentTable.put(leastUsedDoc.getKey(), null);
            ...
            commandFound = removeFromCommandStack(leastUsedDoc, commandFound, tempStack);
        }
    }

    private boolean removeFromCommandStack(Document leastUsedDoc, boolean commandFound, Stack<Undoable> tempStack) {
        while (commandStack.size() > 0) {
            Undoable currentCommand = commandStack.pop();
            if (currentCommand instanceof GenericCommand) {
                if ((((GenericCommand<URI>) currentCommand).getTarget().equals(leastUsedDoc.getKey()))) {
                    commandFound = true;
                    break;
                }
            }else if(currentCommand instanceof CommandSet){
                CommandSet<URI> commandSet = (CommandSet) currentCommand;
                Iterator iterator = commandSet.iterator();
                if (((CommandSet<URI>) currentCommand).containsTarget(leastUsedDoc.getKey())) {
                    while(iterator.hasNext()){
                        if(iterator.next().equals(leastUsedDoc.getKey())){
                            break;
                        }
                    }
                }
    ...
    public Document get(URI url) {
        Document theDoc = documentTable.get(url);
        if (theDoc != null) {
            long newTime = System.nanoTime();
    ...
    public boolean delete(URI url) {
        ...
        if (previousDocument != null) {
            MinHeap<Document> tempHeap = new MinHeapImpl<>();
            removeFromMinHeap(previousDocument, foundDoc, tempHeap);
        ...
        Undoable deleteCommand = new GenericCommand<>(url, (URI thisURI) -> {
            ...
            for(String word : previousDocument.getWords()){
                documentTrie.put(word, previousDocument);
            }
        ...
        while (commandStack.size() > 0) {
            ...
            if (currentCommand instanceof GenericCommand) {
                ...
                if (((CommandSet<URI>) currentCommand).containsTarget(url)) {
                    ...
                    long newTime = System.nanoTime();
                    for (Document doc : documentTable.values()) {
                        if (doc.getKey().equals(url)) {
                            URI theURI = doc.getKey();
                            setNewTime(this.get(theURI), newTime);
                        }
                        //iterator.remove();
                    }
    ...
    public Set<URI> deleteAll(String keyword) {
        ...
        for (Document doc : documentsDeleted) {
            ...
            removeFromMinHeap(doc, foundDoc, tempHeap);
        }
    ...
    private void removeFromMinHeap(Document doc, MinHeap<Document> foundDoc, MinHeap<Document> tempHeap) {
        while (minHeap.peek() != null) {
            Document heapDoc = minHeap.remove();
            if (heapDoc.equals(doc)) {
                foundDoc.insert(doc);
                this.docFormatCheckAndRemove(heapDoc);
                break;
            }
            tempHeap.insert(heapDoc);
        }
        while (tempHeap.peek() != null) {
            Document insertedDoc = tempHeap.remove();
            if (insertedDoc != null) {
    ...
    private void docFormatCheckAndRemove(Document heapDoc) {
    ...
    public Set<URI> deleteAllWithPrefix(String keywordPrefix) {
        ...
        for (Document doc : documentsPrefixDeleted) {
            ...
            documentTable.put(doc.getKey(), null);
            this.removeFromMinHeap(doc, foundHeap, tempHeap);
        }
        for (Document doc : documentsPrefixDeleted) {
